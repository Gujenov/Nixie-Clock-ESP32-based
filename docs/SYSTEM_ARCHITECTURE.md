...
- если DS3231 активен — используется SQW pin (1 Hz)
- иначе — программный таймер ESP32 (`timerAlarmWrite`)

Обработчик `onTimeInterrupt()` только устанавливает флаг обновления и не выполняет тяжёлую логику.
...
Она отвечает за:
- выбор зоны по имени (`Europe/Moscow`, `America/New_York` и т.д.)
