...
1. **Локация обязательна**: пользователь всегда должен указать географическую локацию (например, `Europe/Warsaw`). Пояс по умолчанию в прошивке — `Europe/Warsaw` (UTC+1), см. `DEFAULT_TIMEZONE_NAME` в `include/config.h`. Приведённые ниже примеры с `Europe/Moscow` носят иллюстративный характер; убедитесь, что нужная зона присутствует в таблице `TIMEZONE_PRESETS[]`.
...
4. **Fallback и автокоррекция**: если ezTime недоступен, используется офлайн-источник. При этом действует приоритет: **ezTime (онлайн) → сохранённый POSIX-оверрайд (офлайн) → статическая таблица → UTC**. POSIX-оверрайд — это кэш правил зоны от ezTime, автоматически сохраняемый при обнаружении расхождения с таблицей (см. раздел «Проверка и автокоррекция таблицы»).
...
static const TimezonePreset TIMEZONE_PRESETS[] = {
    ...
    // ... всего 42 предустановленных пояса (PRESETS_COUNT в timezone_manager.cpp)
};
```
...
utcToLocal(time_t utc):
  ├─ РЕЖИМ 1 — ezTime (онлайн):
  │  Если (automatic_localtime && ezTimeAvailable):
  │  ├─ Вызов localTZ.tzTime() → получаем локальное время
  │  ├─ Если ezTime ответил корректно:
  │  │  ├─ Обновляем current_offset и current_dst_active
  │  │  └─ Возвращаем локальное время
  │  └─ Иначе (нет связи) → переходим к режиму 1b
  │
  ├─ РЕЖИМ 1b — офлайн POSIX-оверрайд (автокоррекция):
  │  Если (automatic_localtime && tz_posix задан && tz_posix_zone == timezone_name):
  │  ├─ localTZ.setPosix(tz_posix) → применяем кэшированные правила ezTime
  │  ├─ Обновляем current_offset и current_dst_active
  │  └─ Возвращаем локальное время
  │
  └─ РЕЖИМ 2 — локальная таблица (или fallback):
...
syncTime():
  ...
  3. Установка UTC в систему:
     ...
     └─ rtc->adjust() → DS3231
  4. Сверка правил зоны с ezTime и авто(де)активация POSIX-оверрайда
     (см. раздел «Проверка и автокоррекция таблицы»)
  5. Отключение WiFi
...
## Проверка и автокоррекция таблицы

Когда есть интернет (режим `automatic_localtime = true`), прошивка не просто
доверяет статической таблице, а **проверяет её актуальность по данным ezTime** и,
при необходимости, автоматически подстраивает локальное время. Логика выполняется
при NTP-синхронизации (`src/time_utils.cpp`).

### Что сравнивается

Для текущей зоны сопоставляются два аспекта:

- **Текущий статус** (`current_match`): совпадают ли смещение UTC и признак DST,
  вычисленные по таблице и полученные от ezTime.
- **Правила переходов** (`rules_match`): совпадают ли даты начала/конца DST в
  таблице и у ezTime для текущего года (`compareDSTRulesWithEzTime()`).

### Реакция на результат

```
Если (current_match И rules_match):
  ├─ Лог: "✅ СОВПАДЕНИЕ — локальные правила актуальны"
  └─ clearPosixOverrideIfZone(): удаляем оверрайд для этой зоны (он больше не нужен)

Иначе (обнаружено расхождение):
  ├─ Лог: "⚠️ РАСХОЖДЕНИЕ! Требуется обновление локальных правил"
  ├─ Показываем разницу (ezTime vs таблица)
  └─ savePosixOverride(): сохраняем ПОЛНЫЕ правила зоны от ezTime
```

### Что именно сохраняется

Сохраняется **не числовая «разница», а полная POSIX-строка правил** часового пояса
(стандартное и летнее смещение + правила переходов DST). Она **привязана к
конкретной зоне** и хранится в конфигурации (NVS):

| Поле | Назначение |
|------|------------|
| `tz_posix` | POSIX-строка правил зоны, полученная от ezTime |
| `tz_posix_zone` | Имя зоны, для которой сохранены правила (проверка привязки) |
| `tz_posix_updated` | Время сохранения (UNIX time) |

### Зачем это нужно

Пока таблица `TIMEZONE_PRESETS[]` не обновлена вручную под изменившиеся правила,
сохранённый POSIX-оверрайд позволяет прошивке **автоматически удерживать локальное
время корректным даже без интернета** (режим 1b в `utcToLocal()`, приоритетнее
статической таблицы). Как только таблицу приведут в соответствие с ezTime, при
следующей синхронизации сверка снова покажет совпадение — и оверрайд будет
автоматически снят.

> Важно: механизм работает только в режиме `automatic_localtime = true` и требует
> хотя бы одной успешной онлайн-сверки. В чисто офлайн-режиме
> (`automatic_localtime = false`) всегда используется статическая таблица.
...
- Поиск preset в таблице: O(n) = до 42 итераций
