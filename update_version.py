...
def run_prebuild(env):
    ...
    if success:
        print("[PRE-BUILD] [OK] Версия готова к компиляции\n", flush=True)
    else:
        print("[PRE-BUILD] [WARN] Предупреждение: не удалось обновить версию\n", flush=True)

# Для работы с PlatformIO
if __name__ == "__main__":
    # Если скрипт запущен напрямую (не через PlatformIO)
    # Создаем минимальный объект env с PROJECT_DIR
    class FakeEnv:
        def __init__(self):
            self.data = {'PROJECT_DIR': os.getcwd()}
