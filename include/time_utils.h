...
// ===== ASYNC SYNC (RTOS TASK) =====
void syncTimeAsync(bool force = false, uint8_t preferredNtpIndex = 0);  // Запуск асинхронной синхронизации
bool isSyncInProgress();                       // Проверка, идет ли синхронизация
void processSyncAsync();                       // Неблокирующий шаг асинхронной синхронизации (вызывать из loop)
bool setManualLocalDate(const String &dateStr);  // Установка локальной даты (конвертация в UTC)

// Флаг: время было изменено вручную/по синхронизации и основной цикл должен
// переинициализировать внутреннюю секундантную базу немедленно.
